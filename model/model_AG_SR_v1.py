...
class AG_SR(Module):
    def __init__(self,opt,n_entity,n_relation,n_item):
        ...
        self.linear_three = nn.Linear(self.hidden_size,1,bias=True)
    ...
    def kg_regularization(self, batch_h, batch_t, batch_r):  # tmp for knowledge
    ...
    def get_kg_loss(self,batch_h,batch_t,batch_r):
        h = self.entity_embedding(batch_h)
        t = self.entity_embedding(batch_t)
        r = self.relation_embedding(batch_r)
        r_norm = self.norm_vector(batch_r)
        h_norm = F.normalize(h,p=2,dim=-1)
        h = self.kg_transfer(h,r_norm)
        t = self.kg_transfer(t,r_norm)
        h = F.normalize(h,2,-1)
        r = F.normalize(r,2,-1)
        t = F.normalize(t,2,-1)
        kg_loss = h + (r-t)
        kg_loss = torch.sum(torch.norm(kg_loss,2,-1).flatten())
        return self.kg_loss_rate*kg_loss
    ...
    def predict(self,seq_hiddens,masks,itemindexTensor):
       ...
       ### print('torch.sum(mask,1)',torch.sum(masks,1)[:3]) #batch_size * 1
        ht = seq_hiddens[torch.arange(masks.shape[0]).long(),torch.sum(masks,1) -1] #the last one #batch_size*hidden_size
    ...
    def forward(self, inputs,A):
        hidden = self.entity_embedding(inputs) #batch,L,hidden_size
        hidden = self.gnn(A,hidden) #batch,hidden_size?? !todo
        return hidden
